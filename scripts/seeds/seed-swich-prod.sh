#!/usr/bin/env bash
# One-shot prod seeder for swich-mars. Reads the service-role/secret key from a
# FILE (never a command-line arg or interactive read), so the secret never lands
# in shell history, argv, or a transcript. Validates the key, probes read-only,
# then seeds. Shreds its temp env file on exit.
#
#   1) put ONLY the key in a file (no quotes, no extra lines):
#        cat > /tmp/srk.txt        # paste key, Enter, then Ctrl-D
#      (or just create /tmp/srk.txt in your editor)
#   2) bash scripts/seeds/seed-swich-prod.sh /tmp/srk.txt [--reset]
#
...
KEYFILE="${1:-}"
[ -n "$KEYFILE" ] || { echo "usage: seed-swich-prod.sh <keyfile> [--reset]" >&2; exit 64; }
...
// Raw REST first — gives the true HTTP status + hint.
...
const{data:dup}=await admin.from("workspaces").select("id,name").ilike("name","%Swich%");
