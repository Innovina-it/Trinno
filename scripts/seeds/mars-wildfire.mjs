...
// the ARISE template, with Wildfire's header), gets its [DOCUMENT TITLE] and
...
// strings in word/document.xml (verified against templates/wildfire.docx).
const PATCH_DOCX_PY = `
import sys, zipfile
...
async function setupDrive() {
  ...
  return {
    ...
    // Doc converted from the project template (scripts/seeds/templates/wildfire.docx),
    ...
    async docUrlFor(wpName, docName, meta = {}) {
      ...
      const subtitle =
        [meta.lead, meta.external].filter(Boolean).join(" · ") || wpName;
