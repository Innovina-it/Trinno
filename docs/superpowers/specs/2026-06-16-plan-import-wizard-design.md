...
- A "Pro" model tier / accuracy toggle — v1 is `gemini-3.5-flash` only.
...
- **Gemini client.** `lib/pma/clients/gemini.ts:54` `generateStructured<T>()` — server-only,
  ...
  `gemini-3.5-flash` (`gemini.ts:21`). Currently passes a **text** prompt only
...
- **Model:** `gemini-3.5-flash` only. One call per import over a multi-page PDF; Flash is
